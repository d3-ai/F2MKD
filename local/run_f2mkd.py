...
from simulation.hfl_app import start_mkd_simulation
from hfl_server_app.strategy.fedavg import FedAvg
from fog_app.ray_fog import RayMKDFogServer
...
parser = argparse.ArgumentParser("FedMKD ray simulation using Flower")
...
parser.add_argument("--fog_global_epochs", type=int, required=True, help="The number of epochs for kd from clients")
parser.add_argument("--fog_client_epochs", type=int, required=True, help="The number of epochs for kd from global")
parser.add_argument("--fog_batch_size", type=int, required=True, help="The number of batch_size for local training on fog")
...
parser.add_argument("--alpha", type=float, required=True, help="KD coefficient from server")
parser.add_argument("--beta", type=float, required=True, help="KD from coefficient from clients")
...
def main(args: argparse.ArgumentParser):
    set_seed(args.seed) 
    fog_resources = {"num_cpus": 1}#, "num_gpus": 0.1}
    ...
    def fog_fit_config(rnd: int)->Dict[str,str]:
        config = {
            ...
            "batch_size": str(args.fog_batch_size),
            "global_epochs": str(args.fog_global_epochs),
            "client_epochs": str(args.fog_client_epochs),
            "lr": str(args.lr),
    ...
    hist, params = start_mkd_simulation(
