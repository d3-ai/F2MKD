...
class RayMKDFogServerProxy(FogServerProxy):
    """Flower fogserver proxy which delegates work using Ray."""
    def __init__(self, fog_fn: FogServerFn, fid: str, init_params: Parameters, resources: Dict[str, int], config: Dict[str, Any]):
        super().__init__(fid)
        self.fog_fn = fog_fn
        self.resources = resources
        self.config = config
        self.client_parameters_dict: Dict[str, Parameters] ={
            str(cid): init_params for cid in range(self.config["client_pool"])
        }
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ins.parameters_dict = self.client_parameters_dict
        ...
        self.client_parameters_dict = fit_res.parameters_dict
        fit_res.parameters_dict = None
        return fit_res
