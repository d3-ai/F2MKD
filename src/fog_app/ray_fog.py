...
from driver import train_kd_multiple, train_kd_parameters, test
...
class RayFogServer(FogServer):
    def __init__(self, fid: str, config: Dict[str, Any]) -> None:
        self.fid = fid
        self.config = config
        ...
        self.strategy: FogServerStrategy = FedAvg(
            fraction_fit=strategy_config["fraction_fit"],
            min_available_clients=strategy_config["min_available_clients"],
            on_fit_config_fn=strategy_config["on_fit_config_fn"]   
        )
        ...
        for cid in cids:
            client_proxy = RayClientProxy(
                client_fn=client_config["client_fn"],
                cid=cid,
                resources=client_config["client_resources"],
                config=client_config,
            )
            self.client_manager().register(client=client_proxy)
        log(DEBUG, "Initiate the fogserver")
    ...
    def fit(self, ins: FitIns)->FitRes:# parameters, config) -> FitRes:
        ...
        clinet_instrictions = self.strategy.configure_fit(
            rnd=rnd,
        ...
        log(
            DEBUG,
            "fit_round on fid=%s: FedAvg sampled %s clients (out of %s)",
            self.fid,
            len(clinet_instrictions),
            self._client_manager.num_available(),
        )
        results, failures = self._fit_clients(clinet_instrictions)
        log(
            DEBUG,
            "fogserver fit_round received %s results and %s failures",
            len(results),
            len(failures),
        )
            
        aggregated_result: Union[
            Tuple[Optional[Parameters], Dict[str, Scalar]], Optional[Weights]
        ] = self.strategy.aggregate_fit(rnd, results, failures)
        
        parameters_aggregated, num_examples_aggregated, metrics_aggregated = aggregated_result
        
        return FitRes(
            parameters=parameters_aggregated, num_examples=num_examples_aggregated, metrics=metrics_aggregated
        )
    ...
    def evaluate(self, ins: EvaluateIns) -> EvaluateRes:
        ...
        testloader = DataLoader(testset, batch_size=100, num_workers=num_workers)
        ...
        eval_res = EvaluateRes(
            ...
            metrics={self.fid: float(accuracy)},
        )
        return eval_res
    
    def _get_client_config(self):
        config = self.config["client_config"]
        config["data_dir"] = Path(self.data_dir) / f"C_{self.client_pool}_{self.client_partitions}"
        config["parent_fid"] = self.fid
        return config
...
class RayMKDFogServer(FogServer):
    def __init__(self, fid: str, config: Dict[str, Any]) -> None:
        self.fid = fid
        self.config = config
        self.data_dir = config["data_dir"]
    ...
    def fit(self, ins: FitIns)->FitRes:# parameters, config) -> FitRes:
        ...
        parameters = ins.parameters
        config = ins.config
        client_parameters_dict = ins.parameters_dict
        self.set_parameters(parameters)
        ...
        for client, fit_ins in client_instructions:
            ...
            client_instructions_kd.append(
                (client,FitIns(parameters=client_parameters,config=fit_ins.config))
            )
        
        log(
            INFO,
            "train_kd_from_server() on fogserver fid=%s",
            self.fid
        )

        results, failures = self._fit_clients(client_instructions_kd)
        log(
            DEBUG,
            "fit_round on fid%s: FedMKD received %s results %s and failures",
            self.fid,
            len(results),
            len(failures),
        )
        aggregated_result: Union[
            Tuple[Optional[Dict[str, Parameters]], Dict[str, Scalar]], Optional[Weights]
        ] = self.strategy.aggregate_fit(rnd, results, failures)
        
        parameters_dict_aggregated,_, metrics_aggregated = aggregated_result
        for key, params in parameters_dict_aggregated.items():
            client_parameters_dict[key] = params
        ...
        train_kd_multiple(
            parameters_dict=parameters_dict_aggregated,
        ...
        return FitRes(
            ...
            parameters_dict=client_parameters_dict,
        )
    ...
    def evaluate(self, ins: EvaluateIns) -> EvaluateRes:
        ...
        testloader = DataLoader(testset, batch_size=100, num_workers=num_workers)
        metrics={}
        for id, parameters in ins.parameters_dict.items():
            self.set_parameters(parameters)
            self.model.to(self.device)
            loss, accuracy = test(self.model, testloader, device=self.device)
            cid = int(id) + int(self.fid)* 200
            # log(INFO,"eval() on fogserver cid=%s loss=%s acc=%s",cid, loss, accuracy)
