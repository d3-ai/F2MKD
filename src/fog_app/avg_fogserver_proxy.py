...
from driver import test_parameters
...
class AvgFogServerProxy(FogServerProxy):
    def __init__(self, fid: str, config: Dict[str, Any]) -> None:
        ...
        cids = [str(x) for x in range(self.client_pool)]
        for cid in cids:
    ...
    def fit(self, ins: FitIns)->FitRes:# parameters, config) -> FitRes:
        ...
        log(
            DEBUG,
            "fit_round on fid=%s received %s results and %s failures",
            self.fid,
            len(results),
            len(failures),
        )
...
def fit_clients(
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
        executor.submit(fit_client, client, ins) for client, ins in client_instructions
        ]
        concurrent.futures.wait(futures)
